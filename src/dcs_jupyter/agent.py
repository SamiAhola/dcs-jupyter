...
try:
    ...
    raise ImportError(
        "PydanticAI is required for agent functionality. "
        "Install with: pip install 'dcs-jupyter[agent]'"
    )
try:
    import logfire
...
def spawn_aircraft(
    ...
    lua_template = Template(textwrap.dedent('''
...
Examples:
  ...
  DCS_AGENT_MODEL=gpt-4 python -m dcs_jupyter.agent
    ...
    parser.add_argument(
        '--model', '-m',
        help='AI model to use (preset name or full model identifier)',
        default=None
    )
